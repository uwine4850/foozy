...
func (a *Auth) RegisterUser(username string, password string) error {
	user, err := a.database.SyncQ().Select([]string{"username"}, a.tableName, []dbutils.DbEquals{{"username", username}})
	if err != nil {
		return err
	}
	if len(user) >= 1 {
...
func (a *Auth) UserExist(username string) (map[string]interface{}, error) {
	user, err := a.database.SyncQ().Select([]string{"*"}, a.tableName, []dbutils.DbEquals{{"username", username}})
	if err != nil {
		return nil, err
	}
	if len(user) == 0 {
